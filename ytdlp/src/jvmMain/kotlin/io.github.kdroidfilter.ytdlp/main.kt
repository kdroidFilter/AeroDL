...
fun main() = runBlocking {
    ...
    println("📍 Chemin configuré: ${wrapper.ytDlpPath}")

    // Vérifier si le binaire doit être téléchargé
    if (wrapper.needsDownload()) {
        println("❌ yt-dlp non disponible ou corrompu")
        println("📦 Téléchargement du binaire...")

        val success = withContext(Dispatchers.IO) {
            wrapper.downloadBinary(forceDownload = false)
        }
        ...
        println("✅ yt-dlp déjà disponible")

        // Vérifier s'il y a une mise à jour disponible (optionnel)
        println("🔍 Vérification des mises à jour...")
        val hasUpdate = try {
            wrapper.hasUpdate()
        } catch (e: Exception) {
            println("⚠️ Impossible de vérifier les mises à jour: ${e.message}")
            false
        }

        if (hasUpdate) {
            println("🔄 Une mise à jour est disponible!")
            println("Voulez-vous mettre à jour? (Ignoré pour cet exemple)")
            // Pour forcer la mise à jour, décommenter:
            // val success = withContext(Dispatchers.IO) {
            //     wrapper.downloadBinary(forceDownload = true)
            // }
            // if (success) {
            //     println("✅ Mise à jour installée!")
            // }
        } else {
    ...
    // 2. Afficher la version finale
    val finalVersion = wrapper.version()
    if (finalVersion == null) {
        println("❌ Impossible d'utiliser yt-dlp")
        return@runBlocking
    }
    ...
    // Afficher le chemin du binaire pour référence
    println("💡 Le binaire yt-dlp est stocké dans: ${wrapper.ytDlpPath}")
    println("   Il sera réutilisé lors des prochaines exécutions.")
}
